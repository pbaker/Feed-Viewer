...
FeedGrid = function(viewer, config) {
    ...
    this.feedRecords.push({
        name:'contentType',
        mapping:'media::content@type'
    });
    this.feedRecords.push({
        name:'contentUrl',
        mapping:'media::content@url'
    });
...
// modified to work with mainPanel.loadFeed
// also, baseParams used after store.load, to change tree icon for boxee feeds
...
*/
    ...
    applyRowClass: function(record, rowIndex, p, ds) {
        //debugger; // adds rows of data to the grid
        if (this.showPreview) {
            var xf = Ext.util.Format;
            p.body = '<p>' + xf.ellipsis(xf.stripTags(record.data.description), 100) + '</p>';
